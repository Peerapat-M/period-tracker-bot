...
The only thing that actually fires a due reminder is `scheduler.run_due_jobs()`, exposed at `/run-due-reminders`. **[cron-job.org](https://cron-job.org) hits this URL every 1 minute** — chosen over UptimeRobot (which also has a monitor, but only for `/health`) because UptimeRobot's free tier has a 5-minute floor and cron-job.org's is 1 minute. This lives entirely in cron-job.org's dashboard, not in this repo, so there's nothing here to grep for to confirm it exists. If reminders stop arriving, check that job's execution history there before assuming a code regression — `/health` staying green says nothing about whether reminders are being processed.
